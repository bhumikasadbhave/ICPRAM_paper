...
\author{
    \authorname{
        Bhumika Laxman Sadbhave\sup{1}\orcidAuthor{0009-0004-1129-5729},
        Philipp Vaeth\sup{1}\orcidAuthor{0000-0002-8247-7907},
        Denise Dejon\sup{2},
        Gunther Schorcht\sup{2},
        Magda Gregorová\sup{1}\orcidAuthor{0000-0002-1285-8130}
    }
    \affiliation{\sup{1}Center for Artificial Intelligence and Robotics, Technical University of Applied Sciences Würzburg-Schweinfurt, Franz-Horn-Straße 2, Würzburg, Germany} 
    \affiliation{\sup{2}green spin GmbH, Magdalene-Schoch-Straße 5, Würzburg, Germany}
    \email{bhumikalaxman.sadbhave@study.thws.de, philipp.vaeth@thws.de, \{dejon, schorcht\}@greenspin.de, magda.gregorova@thws.de}
}
...
Sugar-beet cultivation is subject to a wide range of stress factors including, biotic stresses such as diseases and pests, and abiotic stresses like drought, heat, and nutrient deficiencies. 
...
The code used in this study is available at: \url{https://github.com/bhumikasadbhave/Sugar-beet-Stress-Detection-System.git}.
...
Each image includes multiple temporal instances, from which we select seven cloud-free temporal instances spread across the growth season.
An image is represented as $I \in \mathbb{R}^{H \times W \times C \times T}$, where $H=1000$, $W=1000$, $C=10$, and $T=7$ denote the height, width, number of channels, and timepoints, respectively (Figure~\ref{fig:data-repr}).
...
The preprocessing pipeline converts raw Sentinel-2 images into model-ready tensors (Figure~\ref{fig:pre-processing}). 
We first binarize the sugar-beet field ID mask and apply it pixel-wise across all channels and timepoints to retain only the sugar-beet pixels. 
...
\begin{figure}[h]
    ...
    \caption{Data preprocessing overview. The process includes field extraction, removal of cloud-covered temporal instances, and creation of $(4,4)$ sub-patches. Each temporal instance is represented as $(H, W, C)$, and the sequence of seven instances is denoted by $t_1, t_2, \dots, t_7$.}
    \label{fig:pre-processing}
\end{figure}

\begin{table}[t]
\caption{Summary of tensor variants used. Each tensor represents a pre-processed sugar-beet sub-patch across seven temporal instances.}\label{tab:tensor_variants} 
\centering
...
Figure~\ref{fig:rgb_term} shows RGB composites of a single temporal instance for sample sugar-beet fields from the dataset, illustrating the spatial structure and canopy coverage according to the data terminology.
...
The following three subsections outline the modeling process, which comprises feature extraction, downstream clustering, and converting sub-patch predictions to field-level labels.
...
We select seven non-clouded images per field spread across the growth season. However, the acquisition dates vary across sugar-beet fields, and the temporal sequences are not uniformly spaced.
...
The decoder reconstructs the input tensor from the latent representation using transposed 3D convolutions, thus restoring the original temporal and spectral dimensions. 
...
We denote the model as 3D\_AE, with the architecture as shown in Figure~\ref{fig:architecture}.
...
\subsection{Generating Localized Stress Maps}\label{subsec:stress_maps}

Apart from the sugar-beet field predictions, we also generate visual deliverables in the form of field-level stress maps. 
These stress maps are RGB field-level images with localized stress regions highlighted by bounding boxes drawn over the sub-patches predicted as stressed.
...
\begin{figure}[ht]
    ...
    \caption{Localized stress maps for sugar-beet fields as deliverables, marked on RGB composite of the last temporal instance. The first five images correspond to stressed fields, whereas the last image shows a healthy field.}
...
\begin{table*}[t]
    \centering
    \caption{K-means clustering results on sugar-beet fields using various feature extraction methods. The 2D\_AE and 3D\_AE models are assessed with and without temporal encodings.}
...
The evaluation set comprises 61 cloud-free sugar-beet fields corresponding to 48 patches, further divided into 1197 sub-patches.
We use the B10 sub-patch tensor as the default, and sub-patch-to-patch threshold ($\alpha$) as 0.5.
...
While accuracy reflects the overall correctness, the F1 score provides a deeper insight, since the primary focus is on detecting stressed fields while minimizing false positives.
...
The second uses k-means on histogram features computed across all temporal instances. 
The third applies Principal Component Analysis (PCA) on the channel dimension, in order to retain the top three principal components.
The fourth, 2D\_AE, is a 2D convolutional variant of the 3D\_AE model.
It processes input with temporal instances stacked along the channel dimension and omits temporal encodings, resulting in a shape $\mathbb{R}^{H \times W \times C \cdot T}$. 
...
\begin{figure*}[t]
    ...
    \caption{Precision-Recall curve for varying the sub-patch-to-patch threshold ($\alpha$). The baseline attains higher recall at low $\alpha$, but 3D\_AE shows improved precision and F1-score at higher thresholds, indicating greater robustness under stricter labeling conditions.}
...
Figure~\ref{fig:threshold_curve} presents the precision-recall and F1-score curves for the 3D\_AE model and the best-performing baseline (raw data), with $\alpha$ varying from $0.1$ to $1.0$. 
...
We further apply this system to the 2024 season, where high rainfall limited the availability of cloud-free observations.
Four temporal instances (one each from June to September) were used instead of the default seven.
The dataset comprises 2026 unlabeled fields (50529 sub-patches), with 80\% for training and 20\% for testing the autoencoder model. 
...
Despite this imbalance, the system successfully identified the stressed instances from the healthy ones. 
...
As part of this effort, we will explore using temporal data from June to July instead of the full sugar-beet growth cycle from June to September, to facilitate earlier detection.
Additionally, while the current pipeline operates at the sub-patch level, future work will explore pixel-level stress detection for more fine-grained analysis once pixel-level labels become available.
